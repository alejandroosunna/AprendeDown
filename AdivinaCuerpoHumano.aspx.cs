...
public partial class AdivinaCuerpoHumano : System.Web.UI.Page
{
    List<csImagen> Imagenes;
    int IdJuegoAdivina = 1;
            ...
            if (Session["IdRol"] != null && Session["IdUsuario"] != null)
            {
                ...
                else if (IdRol == 2 || IdRol == 3)
                {
                    ...
                    if (!exceptionServices)
                    {
                        ...
                        Response.Redirect("~\\AdivinaCuerpoHumano.aspx?Error=err");
                }
                else
        ...
        if (btnImage.ID == "btnImageUno")
        {
            ...
            if (!exceptionServices)
            {
                if (Imagen.Descripcion == lblParentesco.Text)
                    Response.Redirect("~\\AdivinaCuerpoHumano.aspx?Adivina=ex");
                else
                    Response.Redirect("~\\AdivinaCuerpoHumano.aspx?Adivina=err");
            }
            else
                Response.Redirect("~\\AdivinaCuerpoHumano.aspx?Error=err");
        }
